...
## 2026-05-13 tympanus.net/codrops (Grid to Full Preview)
- 效果名称：网格产品悬停展开预览 (Grid to Full Preview)
- 效果描述：产品网格布局，鼠标悬停某个产品时，对侧区域用 clip-path polygon 动画从小矩形展开为大尺寸预览面板，同时其他产品淡出并向内偏移让出空间，预览面板显示大图、产品名称和价格。包含 4 种变体：① 电商产品展示（家具/装饰品，浅色背景）② 摄影作品集（深色背景，黑白照片）③ 食品菜单（暖色调，餐厅风格）④ 艺术画廊（极简白色，大字排版）。悬停 100ms 后触发，离开时反向收起。
- 实现原理：CSS grid 4列2行布局；预览面板绝对定位覆盖网格，初始 opacity:0；悬停时 JS 驱动：① 预览面板 opacity 1 + clip-path polygon 从中心小矩形展开到全尺寸 ② 其他产品 opacity 0 + translateX/Y 向外偏移；左侧产品在右侧展示，右侧产品在左侧展示（交叉预览）；纯 CSS transition 实现，无需 GSAP 依赖
- 状态：已实现 → 95-grid-hover-preview.html，已集成到 _cat/collect.html（第 51 张卡片）

## 2026-05-13 stripe.com (Radial Network Burst)
- 效果名称：放射线网络动画 (Radial Network Burst)
- 效果描述：从画布中心（底部偏中）向外放射出数十条细线，每条线末端有圆形节点，中心是暖黄/橙色，向外渐变为冷蓝色。线条长度、角度分布不均匀，有呼吸感和扰动动画，节点沿线移动/闪烁。背景是柔和的蓝白径向渐变。包含 4 种变体：① 经典蓝白渐变背景（Stripe 原版风格）② 深色宇宙星空背景（暗色版，线条发光效果）③ 动态节点脉冲（节点随机触发脉冲波纹）④ 互动版（鼠标靠近时射线朝鼠标方向聚拢）
- 实现原理：Canvas 2D API；从底部中心出发，用极坐标生成 40-60 条射线，每条线段分多节绘制（贝塞尔曲线轻微弯曲），线段颜色用 createLinearGradient 从暖色到冷色过渡；节点用 arc() 绘制小圆；用 requestAnimationFrame 驱动时间参数，每条射线有独立的相位偏移实现不同步的呼吸动画；背景用 createRadialGradient 实现柔光晕
- 状态：已实现 → 97-radial-network.html，已集成到 _cat/collect.html（第 52 张卡片）
